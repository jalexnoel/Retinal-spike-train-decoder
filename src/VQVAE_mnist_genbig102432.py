...
def main():
    ...
    myTestDataset = SpikeDataset1(img_path="../dataset/MNIST", transforms=my_transforms, data_type='test')
    ...
    with torch.no_grad():
        for spike, image in myTestDataloader:
            ...
            for i in range(image.shape[0]):
                cv2.imwrite(f"../compare/MNIST/VQVAE_Big102432/gen/image_{image_index}.png",
                            gen_image[i].cpu().numpy().reshape(32, 32, 1) * 255.)
                cv2.imwrite(f"../compare/MNIST/VQVAE_Big102432/src/image_{image_index}.png",
