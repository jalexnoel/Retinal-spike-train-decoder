...
# list = [0,3,5,8,10,16,25,32,33,43,57,63,70,77,90,100,103,115,123,136,146,178,181,197,201,209,236,245,257,260,289,298]
list = [0,3,5,8,10,16,25,32,33,43,57,63,70,77,90,100,103,115,123,136,146]
# list = np.random.randint(0,300,size=20)
...
class MyAllDataset(torch.utils.data.Dataset):
    def __init__(self, archive, transforms = None, is_train = True):
        ...
        for i in range(self.spikedata.shape[0]):
            if i in list:
                self.testspike.append(self.spikedata[i][12])
                self.testimage.append(self.imagedata[i])
                for j in range(12):
                    self.trainspike.append(self.spikedata[i][j])
                    self.trainimage.append(self.imagedata[i])
            else:
                for j in range(13):
                    self.trainspike.append(self.spikedata[i][j])
                    self.trainimage.append(self.imagedata[i])
    ...
    def __getitem__(self, index):
        if self.is_train:
            spike = torch.Tensor(self.trainspike[index])
            if transforms == None:
                image = torch.Tensor(self.trainimage[index])
            else:
                image = self.transforms(torch.Tensor(self.trainimage[index]))
        else:
            spike = torch.Tensor(self.testspike[index])
            if transforms == None:
                image = torch.Tensor(self.testimage[index])
            else:
                image = self.transforms(torch.Tensor(self.testimage[index]))
        return spike, image

    def __len__(self):
        if self.is_train:
            return len(self.trainspike)
        else:
            return len(self.testspike)
...
if __name__ == '__main__':
    ...
    #
    # ])
    # mydataset = SpikeDataset(img_path="../dataset/celebaMask", transforms=my_transforms)
